...
const CollectionPage = () => {
    ...
    const handleOutsideClick = (e) => {
      ...
      if (filterbarRef.current && !filterbarRef.current.contains(e.target) && filterBtnRef.current && !filterBtnRef.current.contains(e.target)) {
        setIsFilterbarOpen(false)
      }

      if (sortbarRef.current && !sortbarRef.current.contains(e.target) && sortBtnRef.current && !sortBtnRef.current.contains(e.target)) {
  ...
  return (
    <div className='flex flex-col lg:flex-row'>
      ...
      <div className='grid grid-cols-2'>
        <button ref={filterBtnRef} onClick={toggleFilterbar} 
        className='lg:hidden border p-2 flex justify-center items-center'>
        <FaFilter className='mr-2'/>Filters</button>

        <button ref={sortBtnRef} onClick={toggleSortbar} 
        className='lg:hidden border p-2 flex justify-center items-center'>
        <FaFilter className='mr-2'/>Sort</button>
      </div>
      

      {/*Filter sidebar */}
      <div ref={filterbarRef} className={`${isFilterbarOpen ? 'translate-x-0' : '-translate-x-full'} fixed inset-y-0 bg-white z-50 w-xs overflow-y-auto transition-transform duration-200 lg:static lg:translate-x-0`}>
        <FilterSidebar/>
      </div>
